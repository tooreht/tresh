...
int lora_mode = 4;         // define the mode wich uses lora
char lora_power = 'H';	   // Send with 7dB Power
...
void loop() {
  ...
  e = sx1272.setMode(lora_mode);  // Select mode: from 1 to 10
  e = sx1272.setCRC_ON();  // Select CRC on or off
  e = sx1272.setPower(lora_power); // Select output power (Max, High or Low)
  ...
  USB.println("Going to sleep for 60s");
  USB.OFF();
  sx1272.OFF();
  PWR.setSensorPower(SENS_5V, SENS_OFF);
  PWR.hibernate("00:00:00:60", RTC_OFFSET, RTC_ALM1_MODE2);
}